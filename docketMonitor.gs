...
function initializeApp() {
  ...
  try {
    ...
    if (appSettings === null) {
      ...
      Utilities.sleep(500);
      appSettings.appendRow(["ignore_parties", "TRUE"]);
      Utilities.sleep(500);
    }
...
function addSettings(property,value) {
  var ss = SpreadsheetApp.getActive();
  var appSettings = ss.getSheetByName("appSettings");
  if (appSettings === null) {
    appSettings = ss.insertSheet();
...
function dmPrimary() {
  ...
  try {
    ...
    if ( (typeof atty === "undefined") || !atty || (atty === 'AR') || (!reEmail.test(attyEmail)) || (!reBarNo.test(barNo)) ) {
      help();
      msg = 'required settings are missing or invalid';
      myLogger(msg);
      var email_error_reports = getSettings("email_error_reports");
      var body = "Please provide a valid name, email address, and bar number to use Docket Monitor.";
      if ( email_error_reports === true ) { MailApp.sendEmail({name: 'Docket Monitor', to: adminEmail, subject: msg, body: body + "\nSheet:\n" + appUrl}); }
      return;
    }
    ...
    if ( send_updates === false ) {
      var subject = "docket monitor case update emails are disabled";
      myLogger(subject);
      var body = "docket monitor case update emails are disabled!";
      var email_error_reports = getSettings("email_error_reports");
      if ( email_error_reports === true ) { MailApp.sendEmail({name: 'Docket Monitor', to: adminEmail, subject: subject, body: body + "\nSheet:\n" + appUrl}); }
    }
  ...
  try {
    ...
    if ( caseNo.length > 0 ) {
      ...
      if ( caseFileId === 'property not found' || !caseFile ) {
        ...
        nCaseText = processHTML(caseResponse);
        nCaseText = processText(nCaseText);
      ...
      /////////////////////////////
      var caseResponseCheck = processRawResponse(caseResponse);
      var pCaseTextCheck = processRawResponse(pCaseText);
      /////////////////////////////
      if ( (caseResponse !== pCaseText) && (caseResponseCheck === pCaseTextCheck) ) {
        ...
        msg = caseNo + ": docket file updated (" + atty + ") [no update sent - no substantive change to docket] ";
        myLogger(msg);
        rStr = "finished";
        return rStr;
      }
      ...
      if (caseResponseCheck !== pCaseTextCheck) {
        ...
        pCaseText = processHTML(pCaseText);
        pCaseText = processText(pCaseText);
        ...
        var subject = caseNo + ' Docket Monitor Update';
        var body = ds + "<br /><hr>" + caseNo + " docket: " + getDataURL + "" + "<br />DocketMonitor: " + SpreadsheetApp.getActive().getUrl();
      ...
      try {
        ...
        if ( blob.getResponseCode() !== 200 ) {
          ...
          if ( prevAttachment === 'property not found' ) {
            ...
            //append a new row with the hash to the settings page
            var date = Utilities.formatDate(new Date(), "America/Chicago", "MM/dd/yyyy");
            appSettings.appendRow([md5Check,date,caseNo]);
          } else {
            //skip this file because it has already been sent in an update
            msg = 'skipping file - sent in previous update ';
            myLogger(msg);
          }
        }
      } catch (err) {
...
function processRawResponse(html) {
